...
import TagImage from "./component/TagImage";
import "./index.css";
import { tagHTML, getRandomRGB, randomRGB } from "./component/TagUtils";
...
const TagInput = () => {
  ...
  const changeBackground = (imageElement) => {
    ...
    <>
      ...
      <div className="container">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              // addNewTag(e.target, DOM.tagContainer); // DOM 변수가 없어서 주석 처리
            }
          }}
        />
        <div className="container_tag">
          <div className="tag" data-keyword="asdfg">
            #검색어
          </div>
        </div>
      </div>
