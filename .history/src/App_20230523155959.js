      ...
      fetch(`https://source.unsplash.com/featured/?${keyword.toLowerCase()}`)
        ...
        .then((response) => {
          ...
          if (response.url === "https://images.unsplash.com/source-404?fit=crop&fm=jpg&h=800&q=60&w=1200") {
            const imageUrl = "https://images.unsplash.com/photo-1470093851219-69951fcbb533?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80";
...
let currentIndex = 0; // 한 번 돌면 위에서 오류가 남
...
button.addEventListener("click", () => {
  const backgroundImageUrl = getComputedStyle(body)
    ...
    .replace(/"/g, ""); // 배경 이미지 URL 추출
  // url("이미지 파일 경로") = slice(4, -1)은 문자열의 첫 4글자(`"url("`)과 마지막 글자 (`")"`를 제거함.) - 배경이미지 url 추출할 때 많이 사용
  // replace는 문자열에서 모든 큰따옴표를 제거
  const link = document.createElement("a"); // a 태그 생성 - 하이퍼링크 설정할때
  link.href = backgroundImageUrl; // 다운로드 링크 지정 - 링크주소 설정할때

  link.download = "background.jpg"; // 파일 이름 지정
  // link.style.display = "none"; // 화면에 표시하지 않도록 설정
  // document.body.appendChild(link); // body에 a 태그 추가
  // link.click(); // 클릭 이벤트 발생

  // <a href=""> 즉 이거 설정해서 이미지 링크로 이동하게 해줌
